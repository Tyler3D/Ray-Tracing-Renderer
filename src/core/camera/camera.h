...
public:
  ...
  inline Ray GetRay(Real s, Real t) const {
        // (1 - t) because (0, 0) is upper right but we want to reorient for lower left
        Vec3r P = this->lower_left_corner_ + s * this->horizontal_ + (1 - t) * this->vertical_;
        /*
        Vec3r Ptemp = P / P.norm();
        Vec3r temp = (P - this->eye_);
        temp = temp / temp.norm();
        std::cout << "Current direction at (s,t) (" << s << ", " << t << ") is (" <<\
        temp[0] << ", " << temp[1] << ", " << temp[2] << ")" << std::endl;

        std::cout << "Point is (" <<\
        Ptemp[0] << ", " << Ptemp[1] << ", " << Ptemp[2] << ")" << std::endl;
        */
        return Ray(this->eye_, P - this->eye_);
  }
