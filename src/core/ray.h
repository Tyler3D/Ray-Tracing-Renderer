...
public:
  ...
  //! \param[in] origin Ray origin
  inline void SetOrigin(const Vec3r &origin) {this->origin_ = origin;}

  //! \brief Set ray direction
  //! \param[in] dir Ray direction
  inline void SetDirection(const Vec3r &dir) {this->dir_ = dir / dir.norm();}

  //! \brief Get ray origin
  //! \return Ray origin
  inline Vec3r GetOrigin() const {return this->origin_;}

  //! \brief Get ray direction
  //! \return Ray direction
  inline Vec3r GetDirection() const {return this->dir_;}
  ...
  inline Vec3r At(Real t) const {return this->origin_ + (t * this->dir_);}
protected:
...
public:
  ...
  inline void SetRayT(Real ray_t) {this->ray_t_ = ray_t;}

  //! \brief Set hit point position
  //! \param[in] point Hit position
  inline void SetPoint(const Vec3r &point) {this->point_ = point;}
  ...
  inline void SetSurface(std::shared_ptr<Surface> surface) {this->surface_ = surface;}

  //! \brief Get ray's fractional distance
  //! \return Ray's fractional distance
  inline Real GetRayT() const {return this->ray_t_;}

  //! \brief Get hit position
  //! \return Hit position
  inline Vec3r GetPoint() const {return this->point_;}

  //! \brief Get surface normal at hit point
  //! \return surface normal
  inline Vec3r GetNormal() const {return this->normal_;}

  //! \brief Return whether the hit point was front or back facing
  //! \return Whether the hit point was front or back facing
  inline bool IsFrontFace() const {return this->front_face_;}

  //! \brief Get hit surface
  //! \return Hit surface
  inline std::shared_ptr<Surface> GetSurface() const {return this->surface_;}
protected:
